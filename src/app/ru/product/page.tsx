...
const DESCRIPTION =
  "7 лет строю магазины на Shopify. Отдаю систему перехода с ВБ и Озона на своё: трафик без рекламы, нужные сервисы, контент на потоке. Забирай бесплатно.";
...
const OLDWAY = [
  ...
  "Клиентов ты не видишь, они площадки, не твои",
  "Соседи демпингуют, и ты режешь цену следом",
];

const SECRETS = [
  { t: "Моя схема органики", d: "Как ловить внимание и продавать без рекламы." },
  { t: "Трафик без бюджета", d: "Один канал вдолгую, который копит аудиторию, а не сгорает." },
  { t: "Инструмент-автопилот", d: "Делает видео из твоих товаров и учится на продажах." },
  { t: "Мой стек инструментов", d: "Чем собираю магазин, ищу товар и поднимаю конверсию." },
];

const STEPS = [
  { n: "1", t: "Забери систему", d: "Два вопроса, почта, доступ сразу." },
  { n: "2", t: "Собери и запусти трафик", d: "По шагам: магазин, который продаёт, и органика без рекламы." },
  { n: "3", t: "Поставь контент на автопилот", d: "Тем же движком держишь объём. Или отдаёшь магазин мне под ключ." },
];

const STACK = [
  "Вся система: магазин и трафик, по шагам",
  "Мои схемы органики и бесплатного трафика",
  "Инструмент-автопилот и как я им пользуюсь",
  "Мой стек инструментов по этапам",
...
const FORYOU = [
  "Строишь или ведёшь магазин и устал гадать по трафику",
...
const FAQ: { q: string; a: string }[] = [
  ...
  { q: "Это надолго бесплатно?", a: "Пока да. Позже могу сделать платным или встроить в продукт побольше. Читаешь сейчас, забирай." },
  { q: "Это очередной курс по дропшиппингу?", a: "Нет. Это система органического трафика на 1 500+ магазинах, мой стек и движок-автопилот. Не 15 часов теории, а то, что двигает деньги." },
    ...
    <main className="min-h-screen bg-white text-zinc-900 selection:bg-lime-400/30 dark:bg-zinc-950 dark:text-zinc-50">
        ...
        <div className="mx-auto grid max-w-6xl items-start gap-8 md:grid-cols-12 md:gap-12">
          <div className="md:col-span-7">
            ...
            <p className="mt-5 max-w-xl text-base leading-relaxed text-zinc-600 dark:text-zinc-400 md:text-lg">
              7 лет строю магазины на Shopify. Отдаю всю систему: как уйти с ВБ и Озона на своё и гнать трафик без вложений в рекламу.
            </p>
      ...
      <section className={SECTION}>
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-6 text-lg font-semibold leading-relaxed text-zinc-900 dark:text-white">Магазин собрать несложно. Сложно, чтобы тебя увидели без оплаты за каждый клик. В этом вся игра, и её я отдаю.</p>
        </div>
      </section>
      ...
      <section className={`${SECTION} border-t border-zinc-200 bg-zinc-50 dark:border-zinc-900 dark:bg-zinc-900/20`}>
        <div className="mx-auto max-w-3xl">
          ...
          <H2>Всё, что обычно прячут за ценником</H2>
          <div className="mt-6 space-y-3">
            {SECRETS.map((s, i) => (
              <div key={s.t} className="flex gap-4 rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
                <div className="font-mono text-lg font-bold text-lime-600 dark:text-lime-400">{String(i + 1).padStart(2, "0")}</div>
                <div><h3 className="font-bold text-zinc-900 dark:text-white">{s.t}</h3><p className="mt-0.5 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">{s.d}</p></div>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="grid items-center gap-8 p-8 md:grid-cols-2 md:p-12">
              <div>
                ...
                <div className="mt-4 space-y-4 text-base leading-relaxed text-zinc-700 dark:text-zinc-300 md:text-lg">
                  <p>Все бросают на одном: делать контент каждый день. Внутри показываю инструмент, который снял эту стену.</p>
